...
import { ReportField, FieldValue, ReportTemplate, FieldType } from '../../models/field.model';
...
export class DynamicFormRendererComponent implements OnInit, OnChanges {
  @Input() template: ReportTemplate | null = null;
  ...
  private initializeFieldStates(): void {
    // Prefer fields input, fallback to template.fields if fields input is empty
    const fieldsToUse = (this.fields && this.fields.length > 0) 
      ? this.fields 
  ...
  onSubmit(event: Event): void {
    ...
    if (hasErrors) {
      ...
      for (let i = 0; i < this.steps.length; i++) {
        const stepHasErrors = this.steps[i].fields.some(f => f.errors.length > 0);
        if (stepHasErrors) {
