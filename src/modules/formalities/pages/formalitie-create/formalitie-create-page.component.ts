    ...
    <div class="create-page">
      ...
      @if (!store.selectedTemplate() && !store.loading() && !store.loadingFields()) {
        <div class="no-template">
          ...
          <p>Por favor regresa y selecciona una plantilla para crear tu formalitie.</p>
          <button class="btn-primary" (click)="goBack()">Regresar a Formalities</button>
        </div>
      }
      ...
      @if (store.selectedTemplate() && store.templateFields().length > 0 && !store.loadingFields()) {
        <div class="form-container">
          <app-dynamic-form-renderer
            ...
            [showActions]="true"
            [submitLabel]="'Guardar Formalitie'"
...
export class FormalitieCreatePageComponent implements OnInit, OnDestroy {
  ...
  async onFormSubmit(values: FieldValue[]): Promise<void> {
    ...
    try {
      await this.store.createAndSubmitFormalitie(this.templateId, values);
      showSuccessAlert('Formalitie creada exitosamente');
      this.router.navigate(['/admin/formalities']);
    } catch (error) {
      showErrorAlert('Error al crear la formalitie. Por favor intente nuevamente.');
    }
  }
